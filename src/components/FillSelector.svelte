<script lang="ts">
  ...
  let unusedTalkFields: string[] =
    Object.keys(activeTalk).filter(publicKeysOnly)
  ...
  const onClick = (key: string, personal = false) => {
    ...
    if (pasteSuccessful) {
      if (!personal) {
        unusedTalkFields = [
          ...unusedTalkFields.filter((field) => field !== key),
        ]
        usedTalkFields = [...usedTalkFields, key]
      } else {
        unusedPersonalFields = [
          ...unusedPersonalFields.filter((field) => field !== key),
        ]
  ...
  <div class="slots">
    <div>
      {#each unusedPersonalFields as unusedField}
        <TalkSuggestion on:click={() => onClick(unusedField, true)}
          >{unusedField}</TalkSuggestion
        >
      {/each}
    </div>
    <hr />
    <div>
      {#each unusedTalkFields as unusedField}
        <TalkSuggestion on:click={() => onClick(unusedField)}
          >{unusedField}</TalkSuggestion
        >
      {/each}
    </div>
        ...
        <div>
          {#if usedPersonalFields.length > 0}
            {#each usedPersonalFields as usedField}
              <TalkSuggestion on:click={() => onClickUsed(usedField, true)}
                >{usedField}</TalkSuggestion
              >
            {/each}
          {/if}
        </div>
        ...
        <div>
          {#if usedTalkFields.length > 0}
            {#each usedTalkFields as usedField}
              <TalkSuggestion on:click={() => onClickUsed(usedField)}
                >{usedField}</TalkSuggestion
              >
            {/each}
          {/if}
