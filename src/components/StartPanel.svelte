...
<section class="flex gap-6 justify-between flex-col p-6 relative">
  <div class="flex flex-col flex-grow relative">
    <Button
      pill
      size="xl"
      class="absolute right-2 top-4 !p-2.5"
      on:click={onClickDashboard}><User /></Button
    >
    ...
    {#if !saveForLater}
      {#if !submitInitialized}
        ...
        {#if lastSyncTime === 'never'}<P
            class="text-gray-500 dark:text-gray-400"
            >Looks like you have not yet set up Github sync or have never
            synced. You can fix that in the dashboard.</P
          >{/if}
        {#if talks.length === 0 && lastSyncTime !== 'never'}<P
            class="text-gray-500 dark:text-gray-400"
            >Looks like you have not added any talks yet. You can do that by
            pushing any suitable markdown files to your repo and then syncing
            again in the dashboard.</P
          >{/if}
        {#if talks.length > 0}
          <TalkSelect
            options={talks}
            bind:value={selectedTalk}
            displayFn={getTalkName}
          />
        {/if}
      {:else if !submitFinished}
        <Heading class="mb-6 mt-3">Way to go!</Heading>
        <P class="text-gray-500 dark:text-gray-400 mb-4"
          >Once you're done filling everything, record your submission with the
          button below before you submit.</P
        >
        <GradientButton
          color="pinkToOrange"
          outline
          pill
          class="mb-6"
          on:click={onDone}>I'm done</GradientButton
        >
      {:else}
        <Heading class="mb-6 mt-3">Outstanding!</Heading>
        <P class="text-gray-500 dark:text-gray-400 mb-4"
          >Your submission has been recorded. You can access the record by
          visiting the dashboard.</P
        >
      {/if}
      ...
      {#if selectedTalk}
        {#if !submitInitialized}
          <GradientButton
            ...
            pill
            class="mb-2"
            on:click={onRoll}>Let's roll</GradientButton
          >
          <DividerWithText class="mb-2">or</DividerWithText>
          <Button
            pill
            color="alternative"
            on:click={onSaveLater}
            outline
            class="mb-6"
          >
