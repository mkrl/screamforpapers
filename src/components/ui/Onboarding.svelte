...
{#if step === 0}
    <Heading class="mb-16">Let's set things up</Heading>
    <Timeline>
        ...
        <TimelineItem title="Create a personal access token" >
            <p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">
                ...
                <Img src={permissionsSelectImage} class="mt-2"  alt="Sreenshot of Contents permissions on GitHub" />
            </p>
        </TimelineItem>
...
{#if step === 1}
    <button on:click={() => step = 0}>Previous</button>
{/if}
