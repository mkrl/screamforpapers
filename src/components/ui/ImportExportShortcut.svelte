<script lang="ts">
    ...
    import {storageLocal, type TalkSubmission, type WishlistItem, type LocalExtensionStorage} from "../../storage";
    ...
    const onExport = () => {
        ...
        downloadAnchorNode.setAttribute("download", `${dataType}-${new Date().getTime()}.json`);
    ...
    $: if (files) {
        files[0].text().then((text) => {
            try {
                const parsedItems = dataType === 'submissions'
                    ? validateSubmissions(JSON.parse(text))
                    : validateWishlist(JSON.parse(text))
                if (parsedItems.length > 0) {
                    const importData = dataType === 'submissions'
                    ...
                    alert(`No valid ${dataType} found in the file you are trying to import`);
                }
            } catch (e) {
