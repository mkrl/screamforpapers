...
export const syncWithGithub = async ({ token, repo, logFn = console.log }: { token: string, repo: string, logFn?: (log: string) => void }) => {
    if (token && repo) {
        ...
        try {
            ...
            for (const file of repoData) {
                if (file.name.endsWith(".md")) {
                    try {
                        ...
                        logFn(`Error while fetching ${file.name}, perhaps it's worth retrying later`)
                        throw e
                    }
                }
            }
            ...
            logFn(`Error while fetching the repository, is your token valid and has read access?`)
            throw e
        }
